<template>
  <div class="page-container">
    <div class="page-header">
      <el-form inline>
        ...
        <el-form-item>
          ...
          <el-button type="success" icon="el-icon-plus" @click="handleAdd">新增电费</el-button>
        </el-form-item>
      </el-form>
    </div>
    ...
    <el-dialog :title="editForm.id ? '编辑电费' : '新增电费'" :visible.sync="dialogVisible" width="500px">
      <el-form ref="editForm" :model="editForm" :rules="formRules" label-width="90px">
        <el-form-item label="房屋" prop="houseId">
          <el-select v-model="editForm.houseId" placeholder="请选择房屋" filterable>
...
<script>
import { powerFeeApi, roomApi, liveUserApi } from '@/api'
...
export default {
  ...
  methods: {
    async loadData() {
      const res = await powerFeeApi.list(this.query)
    ...
    async submitForm() {
      this.$refs.editForm.validate(async valid => {
        if (!valid) return
        if (this.editForm.id) await powerFeeApi.update(this.editForm)
        else await powerFeeApi.add(this.editForm)
    ...
    async handleDelete(row) {
      await this.$confirm('确认删除该电费记录？', '提示', { type: 'warning' })
      await powerFeeApi.delete(row.id)
